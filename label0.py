...
def HeadTail(num_f_classes, alpha, num_c_classes=None, relation_f=None):
    tail_f_marks = [i for i in range(num_f_classes) if i >= num_f_classes * (1 - alpha / 100)]
    head_f_marks = [i for i in range(num_f_classes) if i not in tail_f_marks]
    ...
    if num_c_classes:
        ...
        return head_f_marks, tail_f_marks, head_c_marks, tail_c_marks
    return head_f_marks, tail_f_marks


# ==================================================================
def TailFineToCoarse(f_labels, hf_labels, tf_labels, relation_f):
    c_labels = []

    for i in range(len(f_labels)):
        if f_labels[i] in tf_labels:
            c_labels.append(relation_f[f_labels[i]] + len(hf_labels))
        else:
...
def TailFineToCoarse_miss(relation_f):
    a, b = np.unique(np.array(relation_f), return_counts=True)
    if a[len(a) - 1] != len(a) - 1:  # isMiss.  isMiss: [0, 1, 3, 4, 5] -> noMiss: [0, 1, 2, 3, 4]
        ...
        for i in miss:
            for j in range(len(relation_f)):
                if i < relation_f[j]:
                    relation_f[j] = relation_f[j] - 1
...
def Sort_HeadTail(train_labels):
    num_perclass_dataset, num_perclass_train, num_perclass_test, ratio_test_train = \
        dataset0.NumPerclass(train_labels, '')
