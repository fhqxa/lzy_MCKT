...
def AllDatasets():
    ...
    if 'CIFAR' in switch_dataset:
        ...
        labels_c = [i[2] for i in image_path]
        num_f_classes, num_c_classes, relation_f = label0.RelationFine1(labels, labels_c)
    ...
    if 'VOC2012' in switch_dataset:
        ...
        labels = [i[1] for i in image_path]
        a, b = label0.Sort_HeadTail(labels)
        labels = [i - a[np.where(b == i)[0][0]] for i in labels]
        for i in range(len(image_path)):
            image_path[i][1] = labels[i]
        ...
        image_path_test = [[os.path.join(leaves_path_test[i], j), i, coarse_layer[i]]
                           for i in range(len(leaves_path_test)) for j in os.listdir(leaves_path_test[i])]
        labels = [i[1] for i in image_path_test]
        labels = [i - a[np.where(b == i)[0][0]] for i in labels]
        for i in range(len(image_path_test)):
            image_path_test[i][1] = labels[i]
    ...
    return image_path, image_path_test, transform_train, transform_test, num_f_classes, num_c_classes, relation_f, \
           eval_tree
...
def IndexFromDataset_resampling(dataset, num_perclass):
    length = dataset.__len__()
    num_perclass = num_perclass.copy()
...
def LongTailDistribution(num_perclass_train, ratio_longtail=100):
...
def NumPerclass(train_labels, test_labels):
    if test_labels == 'traintest':
        a, b = np.unique(train_labels, return_counts=True)
        # print(a)
        # print(b)
        num_classes = len(a)
        num_perclass_dataset = b.tolist()
        ...
        #         print('no sort')
        num_headclass = num_perclass_dataset[0]
        num_tailclass = num_perclass_dataset[num_classes - 1]
        # if num_headclass > 2 * num_tailclass:
        #     print('the original dataset is imbalanced')

        ratio_test_train = 5  # train / test = 5 / 1
        num_perclass_train = [int(i * ratio_test_train / (1 + ratio_test_train)) for i in num_perclass_dataset]
        num_perclass_test = [i - j for i, j in zip(num_perclass_dataset ,num_perclass_train)]
    else:
        num_perclass_dataset, ratio_test_train = '', ''

        a, b = np.unique(train_labels, return_counts=True)
        # print(a)
        # print(b)
        num_classes = len(a)
        num_perclass_train = b.tolist()
        # print(num_perclass_dataset, num_classes)
        # for i in range(len(num_perclass_dataset) - 1):
        #     if num_perclass_dataset[i] < num_perclass_dataset[i + 1]:
        #         print('no sort')
        num_headclass = num_perclass_train[0]
        num_tailclass = num_perclass_train[num_classes - 1]
        # if num_headclass > 2 * num_tailclass:
        #     print('the original dataset is imbalanced')
    ...
    if 'LT' in switch_imbalanced:
        num_perclass_train = LongTailDistribution(num_perclass_train, ratio_LT)
    if switch_imbalanced == 'DecreaseToTail':
        if num_headclass > 2 * num_tailclass:
            num_perclass_train = [num_tailclass] * num_classes
        else:  # the original dataset is balanced
            num_perclass_train = [num_tailclass / ratio_LT] * num_classes

    return num_perclass_dataset, num_perclass_train, num_perclass_test, ratio_test_train


def NumPerclass_Coarse(num_f_classes, num_c_classes, relation_f, num_perclass_train):
    num_perclass_train_c = [0] * num_c_classes
    for i in range(num_f_classes):
        c = relation_f[i]
        n = num_perclass_train[i]
        num_perclass_train_c[c] += n
    # print(num_perclass_train_c)
    # print(sum(num_perclass_train_c))

    return num_perclass_train_c
...
def DataloaderFromDataset(train_dataset, test_dataset, num_perclass_train, num_perclass_test, switch_resampling,
                          defer_epoch, batch_size, epoch):
    if switch_resampling == 'Resampling' and epoch == defer_epoch:
        train_idx = IndexFromDataset_resampling(train_dataset, num_perclass_train)
    ...
    elif switch_resampling == 'SMOTE' and epoch == defer_epoch:
        aug_data, aug_label = SMOTE(train_dataset.image, train_dataset.label,
